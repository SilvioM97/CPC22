...
pub fn heap(nums: &Vec<i32>, k: i32) -> Vec<i32> {
    ...
    for i in (k - 1)..n {
        ...
        while !cont {
            if ((heap.peek()).unwrap()).1 >= (i - k + 1) {
                //Check if the max is (strictly) in the window
                maximums.push(((heap.peek()).unwrap()).0); //Take the max
                cont = true; //Set cont to true so we go to the next iteration
            } else if ((heap.peek()).unwrap()).1 == i - k + 1 {
                //Check if the max is in the first position of the window
    ...
    for i in 0..n {
        while !cont {
            if !deq.is_empty() && i >= k && (*((deq.front()).unwrap())).1 < (i - k + 1) {
                //If the deque is not empty remove the head element if it's not in the window and if we have already loaded the first window
                deq.pop_front();
            } else {
